...
\gard{I suggest replacing this paragraph with just: Let $C^p(K)$ be the set of functions $K_p\to\RR$ with the obvious vector space structure. These \emph{$p$-cochains} will encode our data. Define the linear \emph{coboundary} maps by $\delta^p:C^p\to C^{p+1}$}
\begin{equation*}
