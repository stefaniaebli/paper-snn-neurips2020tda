...
Lausanne, Switzerland\\
  \texttt{stefania.ebli@epfl.ch} \thanks{The author was supported by the Swiss National Science Foundation grant number 200021\_172636.}\\
...
\begin{enumerate}
    ...
    \item \mdeff{The saved space should be used to write more about the (co)boundary, and the dual (co)differential. The duality of taking the border or the derivative should be intuitive to anyone. Also add $B_0$ and/or $B_1$ to Figure 1. (If space allows, we could link to div/grad/curl for more intuition.)}\stefania{At the beginning the section about (co)boundary was longer. I started by defining the boundary and then the coboundary. I then realized it was too long and decided to cut it. Since the NeurIPS workshop is in TDA, I am sure the reviewer are familiar with these notions. However, for a longer version of the article I would definitely add a more detailed explanation and intuitive figures.}
    \item \mdeff{Use it to explain how we link cochains of different degree. Maybe add a diagram of the SNN. (BTW we might want to consider the antiderivative $B^+$ instead of the codifferential $B\transpose$ to go down.)}\stefania{Same as point 2. Maybe we can add a sentence about how to link different degree in future work? I think it would be great having a diagram of SNN but for space constrain it won't fit (we tried in a preliminary version and it was too much). Also to be added longer version it should be there.}
\end{enumerate}
Minor:
\begin{itemize}
    \item \mdeff{Can we have the figures in vector graphics (e.g., PDF) rather than PNG?}\stefania{Sure, I was planning to replot each figure in pdf format as as soon as the writing was done.}
    ...
    \item \mdeff{Check for consistent terminology, e.g., node vs vertex.}\stefania{I'll do this. (Fixed: vertex)}
\end{itemize}
\clearpage
