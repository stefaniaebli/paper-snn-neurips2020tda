...
Minor:
\begin{itemize}
    \item \mdeff{Can we have the figures in vector graphics (e.g., PDF) rather than PNG?}
    \item \mdeff{I've done it here and there, but still more space can be saved by tightening the writing.}
    \item \mdeff{Shall we use such a generic title? We may want to write a more comprehensive paper in the future.}
    \item \mdeff{There's a mix of Title Case and sentence case in section or paragraph headers. We should stick to one.}
    \item \mdeff{Check for consistent terminology, e.g., node vs vertex.}
\end{itemize}
\clearpage
