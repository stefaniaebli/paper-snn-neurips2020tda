...
\tikzset{twosimpred/.style={fill opacity=0.6,fill=red,draw opacity=0.9}}
...
\begin{columns}
  ...
  \column{0.5}{  
    ...
    \block{Motivation: beyond pairwise interactions}{
\vspace{-0.5cm}
In \cite{1} CNNs have been extended to convolutional neural networks on graphs (GNNs).
\vspace{0.5cm}
\begin{center}
\begin{minipage}{0.4\linewidth}
\vspace{-1cm}
...
\vspace{-1cm}
\begin{itemize}
\item Graphs model pairwise relationships. We extend GNNs to \textbf{simplicial complexes}, which can encode $k$-fold interactions.
\item The input of simplicial neural networks (SNNs) are $\mathbf{k}$\textbf{-cochains (functions)} on the $k$-simplices.
\item We use the \textbf{simplicial Laplacian} to define a proper notion of convolution.
\end{itemize}
...
\vspace{0.5cm}
From any bipartite graph with weights one can build a simplicial complex with $k$-cochains.
\vspace{1.5cm}
...
\vspace{1cm}
\begin{itemize}
\item From the the Semantic Scholar Dataset we built two different coauthorship complexes $CC1$ and $CC2$.
\end{itemize}
} 
...
We consider the problem of \textbf{imputing missing values of citations} on the cochains ($k=1,2,3$) of a coautorship complex. We train a SNN bala bla
\vspace{0.3cm}
...
\begin{center}
  \includegraphics[height=12cm]{figures/foo-transfer.pdf}
 \end{center}
\end{minipage}
